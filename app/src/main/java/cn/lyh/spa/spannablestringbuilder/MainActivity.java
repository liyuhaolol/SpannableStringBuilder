package cn.lyh.spa.spannablestringbuilder;
...
public class MainActivity extends Activity {
	...
	private void setSpannableSBText() {
		...
		ssb.setSpan(new BackgroundColorSpan(Color.GRAY), new String("设置字体").length(), new String("设置字体背景色 ").length(),
				Spanned.SPAN_EXCLUSIVE_EXCLUSIVE); // 设置背景色为青色
		spannablesbList.add(ssb);

		ssb = new SpannableStringBuilder("设置字体背景色 Long值方式");
		ssb.setSpan(new BackgroundColorSpan(line_color), new String("设置字体").length(), new String("设置字体背景色 ").length(),
				Spanned.SPAN_EXCLUSIVE_EXCLUSIVE); // 设置背景色为青色
		spannablesbList.add(ssb);
		...
		//
		ssb.setSpan(new SuperscriptSpan(), new String("设置上下标:y=x").length(), new String("设置上下标:y=x2").length(),
				Spanned.SPAN_EXCLUSIVE_EXCLUSIVE); // 下标
		ssb.setSpan(new SubscriptSpan(), new String("设置上下标:y=x3+a").length(), new String("设置上下标:y=x3+an").length(),
		...
		ssb.setSpan(new URLSpan("cacaca") {
			...
			public void onClick(View widget) {
				Toast.makeText(MainActivity.this, "点击了设置的链接", Toast.LENGTH_SHORT).show();
			}
		...
		ssb.setSpan(new BulletSpan(BulletSpan.STANDARD_GAP_WIDTH, Color.GREEN), 0,
				new String("设置项目符号").length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE); // 第一个参数表示项目符号占用的宽度，第二个参数为项目符号的颜色
		spannablesbList.add(ssb);
		...
		ssb.setSpan(new TypefaceSpan("default"), 0, new String("设置字体(default,").length(),
				Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
		ssb.setSpan(new TypefaceSpan("default-bold"), new String("设置字体(default,").length(),
				new String("设置字体(default,default-bold,").length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
		ssb.setSpan(new TypefaceSpan("monospace"), new String("设置字体(default,default-bold,").length(),
				new String("设置字体(default,default-bold,monospace,").length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
		ssb.setSpan(new TypefaceSpan("serif"), new String("设置字体(default,default-bold,monospace,").length(),
				new String("设置字体(default,default-bold,monospace,serif,").length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
		ssb.setSpan(new TypefaceSpan("sans-serif"), new String("设置字体(default,default-bold,monospace,serif,").length(),
				new String("设置字体(default,default-bold,monospace,serif,sans-serif)").length(),
				Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
		spannablesbList.add(ssb);

		ssb = new SpannableStringBuilder("设置字体大小(绝对值：单位:像素/单位:像素)");
		ssb.setSpan(new AbsoluteSizeSpan(20), new String("设置字体大小(绝对值,").length(),
				new String("设置字体大小(绝对值,单位:像素,").length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
		ssb.setSpan(new AbsoluteSizeSpan(20, true), new String("设置字体大小(绝对值,单位:像素,").length(),
				new String("设置字体大小(绝对值,单位:像素,单位:像素)").length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE); // 第二个参数boolean
		// dip，如果为true，表示前面的字体大小单位为dip，否则为像素，上同
		spannablesbList.add(ssb);

		ssb = new SpannableStringBuilder("设置字体大小（相对值：一半/两倍,单位：像素） 参数表示为默认字体大小的多少倍 ");
		ssb.setSpan(new RelativeSizeSpan(0.5f), new String("设置字体大小（相对值：").length(),
				new String("设置字体大小（相对值：一半/").length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE); // 0.5f表示默认字体大小的一半
		ssb.setSpan(new RelativeSizeSpan(2.0f), new String("设置字体大小（相对值：一半/").length(),
				new String("设置字体大小（相对值：一半/两倍,").length(), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE); // 2.0f表示默认字体大小的两倍
		spannablesbList.add(ssb);
		String aa = "我的后面添加图片";
		aa = aa + " ";
		ssb = new SpannableStringBuilder(aa);
		ssb.setSpan(new ImageSpan(this, R.mipmap.ic_launcher), aa.length() - 1, aa.length(),
		...
		ssb.setSpan(new ImageSpan(this, R.mipmap.ic_launcher), 0, 1, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
